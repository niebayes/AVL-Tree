...
class AVL {
 ...
 private:
  int get_height(Node* x) {
    ...
    return std::max(get_height(x->left), get_height(x->right)) + 1;
  }
  ...
  Node* rebalance(Node* x) {
    ...
    if (x_bf > 0) {
      // left heavy, insertion was in node x's left subtree.
      ...
      if (y_bf > 0) {
        // left heavy, insertion was in node y's left subtree.
        return right_rotate(x);

      } else {
        // right heavy, insertion was in node y's right subtree.
        return left_right_rotate(x);
      }

    } else {
      // right heavy, insertion was in node x's right subtree.
